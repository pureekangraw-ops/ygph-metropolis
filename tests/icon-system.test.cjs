...
test("icon rollout stays wired to the current Metropolis cache generation", () => {
  const manifest = JSON.parse(read("RELEASE_MANIFEST.json"));
  const { RELEASE_ID } = require("../sw.js");

  assert.equal(manifest.serviceWorker.releaseId, RELEASE_ID);
  assert.match(RELEASE_ID, /^v4\.2\.1-20260808-r6-status$/, "the status patch must advance beyond the schedule cache");
});

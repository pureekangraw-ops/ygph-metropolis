...
test('PIN authentication errors use PIN-facing language without changing credential semantics', async () => {
  const { authMessage } = await import(runtimeGatePath);

  assert.equal(authMessage(new Error('DEVICE_PIN_INVALID')), 'PIN ไม่ถูกต้อง');
  assert.match(authMessage(new Error('DEVICE_PIN_TOO_SHORT')), /^PIN ต้องมีอย่างน้อย \d+ ตัวอักษร$/);

...
test('command runtime applies one named domain command and increments revision once', async () => {
  ...
  runtime.register('STORE', 'STORE_ADD_RECORD', ({ domainState, payload }) => {
    domainState.records[payload.recordId] = { record: { recordId: payload.recordId }, provenance: { origin: 'LIVE_COMMAND' } };
  });
  const state = createGreenfieldState();
  const next = await runtime.execute(state, { commandId: 'C1', idempotencyKey: 'K1', domain: 'STORE', type: 'STORE_ADD_RECORD', expectedRevision: 1, payload: { recordId: 'S1' } });
  assert.equal(next.revision, 2);
  assert.equal(next.domains.STORE.records.S1.record.recordId, 'S1');
  assert.equal(next.commandLog.K1.commandId, 'C1');
});

test('command runtime rejects stale revision, duplicate idempotency key, and cross-domain registration', async () => {
  const { createGreenfieldState } = await import('../greenfield/core.mjs');
  const { createCommandRuntime } = await import('../greenfield/command-runtime.mjs');
  const runtime = createCommandRuntime();
  runtime.register('LEDGER', 'LEDGER_NOOP', () => {});
  const state = createGreenfieldState();

...
export function resolveCalendarAction(state, queue) {
  ...
  if (OBLIGATION_TYPES.has(queue.type)) {
    ...
    return { available:true, kind:'PAY_OBLIGATION', owner:'LEDGER', sourceId:relation.source.recordId, method:'payObligation', maxAmountSatang:Math.min(queueRemaining, sourceRemaining), queueId:queue.recordId };
  }
  ...
  if (queue.type === 'RECEIVE_CUSTOMER_PAYMENT') {
    ...
    const sourceRemaining = remainingFor(relation.source, relation.owner);
    if (!Number.isSafeInteger(queueRemaining) || queueRemaining <= 0 || !Number.isSafeInteger(sourceRemaining) || sourceRemaining <= 0) return { available:false, reason:'SOURCE_AMOUNT_INVALID' };
    return { available:true, kind:'RECEIVE_CUSTOMER_PAYMENT', owner:'STORE', sourceId:relation.source.recordId, method:'receiveCustomerPayment', maxAmountSatang:Math.min(queueRemaining, sourceRemaining), queueId:queue.recordId };
  }

...
function permissions(manifestText) {
  return [...manifestText.matchAll(/<uses-permission\b[^>]*android:name="([^"]+)"[^>]*\/?\s*>/gs)].map(match => match[1]).sort();
}
...
function actions(body) {
  return [...body.matchAll(/<action\b[^>]*android:name="([^"]+)"[^>]*\/?\s*>/gs)].map(match => match[1]).sort();
}
function components(manifestText) {
  ...
  for (const tagName of COMPONENT_TAGS) {
    ...
    for (const match of manifestText.matchAll(paired)) {
      ...
      const body = match[2];
      found.push({ type:tagName, name:attr(open, 'name') || '', exported:boolAttr(open, 'exported'), permission:attr(open, 'permission') || null, actions:actions(body), launcher:/android\.intent\.action\.MAIN/.test(body) && /android\.intent\.category\.LAUNCHER/.test(body), hasIntentFilter:/<intent-filter\b/.test(body) });
    }
    const selfClosing = new RegExp(`<${tagName}\\b([^>]*)\\/\s*>`, 'g');
    for (const match of manifestText.matchAll(selfClosing)) {
      const open = `<${tagName}${match[1]}/>`;
      found.push({ type:tagName, name:attr(open, 'name') || '', exported:boolAttr(open, 'exported'), permission:attr(open, 'permission') || null, actions:[], launcher:false, hasIntentFilter:false });
    }
  }
...
function enabledPluginSurface(capacitorConfig) {
  ...
  return Object.entries(plugins).filter(([, value]) => !(value && typeof value === 'object' && value.enabled === false)).map(([name]) => name).sort();
}
...
export function inspectAndroidSecurity({ manifestText, capacitorConfig, manifestPath = null }) {
  ...
  return { status:'PROVEN', manifestPath, applicationId:packageId || configAppId || null, capacitorAppId:configAppId || null, requestedPermissions, declaredPermissions:permissionDeclarations, components:componentInventory, exportedComponents, backupPolicy:{ allowBackup:allowBackup ?? null }, networkPolicy:{ usesCleartextTraffic:usesCleartextTraffic ?? null }, debuggable, enabledNativePluginSurface:enabledPluginSurface(capacitorConfig) };
}

export function verifyAndroidSecurity(input) {
  const evidence = inspectAndroidSecurity(input);
  if (!evidence.applicationId || evidence.applicationId !== EXPECTED_APPLICATION_ID) throw new Error(`ANDROID_SECURITY_APPLICATION_ID_MISMATCH:${evidence.applicationId || 'UNKNOWN'}`);
  if (evidence.capacitorAppId && evidence.capacitorAppId !== EXPECTED_APPLICATION_ID) throw new Error(`ANDROID_SECURITY_CAPACITOR_APP_ID_MISMATCH:${evidence.capacitorAppId}`);
  ...
  for (const component of evidence.components) {
    if (component.hasIntentFilter && component.exported === undefined) throw new Error(`ANDROID_SECURITY_EXPORTED_POLICY_UNKNOWN:${component.type}/${component.name || 'UNKNOWN'}`);
    if (component.type === 'provider' && component.exported !== false) throw new Error(`ANDROID_SECURITY_PROVIDER_NOT_PRIVATE:${component.name || 'UNKNOWN'}`);
  }
  return evidence;
}
...
async function findMergedReleaseManifest(androidRoot) {
  ...
  const matches = allManifests.filter(path => /\/merged_manifests?\/release\//.test(path.replaceAll('\\', '/')));
...
async function readGeneratedCapacitorConfig(androidRoot) {
  ...
  catch (error) { if (error?.code === 'ENOENT') throw new Error('ANDROID_SECURITY_GENERATED_CAPACITOR_CONFIG_MISSING'); throw error; }

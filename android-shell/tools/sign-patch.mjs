...
function parseVersion(value, label) {
  if (typeof value !== 'string' || !SEMVER.test(value)) throw new Error(`${label} must be a numeric SemVer triplet`);
...
function pemToBytes(pemText) {
  ...
  const body = pemText.replace(/-----BEGIN [^-]+-----/gu, '').replace(/-----END [^-]+-----/gu, '').replace(/\s+/gu, '');
...
export async function signPatchSource({ source, privateKeyPem, keyId = DEFAULT_KEY_ID }) {
  ...
  for (const path of paths) {
    ...
    if (canonicalTargets.has(canonicalPath)) throw new Error(`Duplicate canonical patch asset path: ${canonicalPath}`);
  ...
  const bundle = {
    ...
    files:entries,
    signature:{ alg:PATCH_ALGORITHM, keyId, value:'' },
  };
  let privateKey;
  try { privateKey = await webcrypto.subtle.importKey('pkcs8', pemToBytes(privateKeyPem), { name:'ECDSA', namedCurve:'P-256' }, false, ['sign']); }
  catch { throw new Error('Private patch signing key is invalid'); }
  const signature = await webcrypto.subtle.sign({ name:'ECDSA', hash:'SHA-256' }, privateKey, encoder.encode(canonicalPatchPayload(bundle)));
...
async function main(argv) {
  ...
  if (!inputPath || !privateKeyPath || !outputPath) throw new Error('Usage: npm run patch:sign -- <input-json> <private-key.pem> <output.lhpatch>');
  const [sourceText, privateKeyPem] = await Promise.all([readFile(inputPath, 'utf8'), readFile(privateKeyPath, 'utf8')]);

...
export async function applyAndroidVersion({
  ...
  if (!Number.isInteger(version.versionCode) || version.versionCode <= 0) throw new Error('APK_VERSION_CODE_INVALID');
  if (typeof version.versionName !== 'string' || version.versionName.trim() === '') throw new Error('APK_VERSION_NAME_INVALID');
  ...
  if (after === before || !after.includes(`versionCode ${version.versionCode}`) || !after.includes(`versionName "${version.versionName}"`)) throw new Error('APK_VERSION_APPLY_FAILED');
  await writeFile(gradlePath, after, 'utf8');
  const readback = await readFile(gradlePath, 'utf8');
  if (!readback.includes(`versionCode ${version.versionCode}`) || !readback.includes(`versionName "${version.versionName}"`)) throw new Error('APK_VERSION_READBACK_MISMATCH');
  return version;
}

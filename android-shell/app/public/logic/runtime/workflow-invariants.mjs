...
function validateVerifiedExpenseRelation(state, commands) {
  const ledgerWrites = commands.filter(command => command?.domain === 'LEDGER' && command?.type === 'LEDGER_CREATE_TRANSACTION' && command?.payload?.subtype === 'VERIFIED_EXPENSE');
  if (ledgerWrites.length === 0) return;
  const completions = commands.filter(command => command?.domain === 'CALENDAR' && command?.type === 'CALENDAR_SET_STATUS' && command?.payload?.status === 'COMPLETED');
  ...
  if (completions.length === 1 && String(completions[0]?.payload?.recordId || '') !== queueId) throw new Error(`WORKFLOW_VERIFIED_EXPENSE_SOURCE_MISMATCH:${String(completions[0]?.payload?.recordId || '')}/${sourceRef}`);
  const queue = state?.domains?.CALENDAR?.records?.[queueId]?.record;
  if (!queue) throw new Error(`WORKFLOW_QUEUE_NOT_FOUND:${queueId}`);
  const amount = Number(queue.amountSatang ?? 0);
  if (queue.type !== 'VERIFY' || String(queue.ownerRef || '').toUpperCase() !== 'LEDGER' || !Number.isSafeInteger(amount) || amount <= 0) throw new Error(`WORKFLOW_VERIFIED_EXPENSE_QUEUE_INVALID:${queueId}`);
  if (completions.length === 0 && queue.status !== 'COMPLETED') throw new Error(`WORKFLOW_VERIFIED_EXPENSE_COMPLETION_REQUIRED:${queueId}`);
  if (completions.length === 1 && (queue.status === 'COMPLETED' || queue.status === 'CANCELLED')) throw new Error(`WORKFLOW_VERIFIED_EXPENSE_QUEUE_CLOSED:${queueId}/${queue.status}`);
  const alreadyLinked = recordsFor(state, 'LEDGER').some(record => record?.type === 'TRANSACTION' && record?.direction === 'OUT' && ledgerSubtype(record) === 'VERIFIED_EXPENSE' && record?.sourceRef === `CALENDAR/${queueId}` && record?.status !== 'REVERSED');

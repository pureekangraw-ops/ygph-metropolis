...
function base64urlToBytes(value) {
  if (typeof value !== 'string' || value.length === 0) throw new Error('Patch signature value is required');
  if (!/^[A-Za-z0-9_-]+$/u.test(value)) throw new Error('Patch signature is invalid');
  const padded = `${value.replaceAll('-', '+').replaceAll('_', '/')}${'='.repeat((4 - (value.length % 4)) % 4)}`;
  let binary;
  try { binary = globalThis.atob(padded); } catch { throw new Error('Patch signature is invalid'); }
...
export function canonicalPatchPayload(bundle) {
  ...
  const canonicalFiles = Object.keys(files).sort().map(path => ({ path, sha256:files[path]?.sha256 }));
  return JSON.stringify({ schema:source.schema, baseVersion:source.baseVersion, version:source.version, files:canonicalFiles });
}

export async function verifyPatchBundle(bundle, { currentVersion, trustedKey } = {}) {
  const source = asObject(bundle, 'Patch bundle');
  const key = asObject(trustedKey, 'Trusted patch key');
  if (source.schema !== PATCH_SCHEMA) throw new Error(`Patch schema must be ${PATCH_SCHEMA}`);
  ...
  if (source.baseVersion !== currentVersion || compareVersion(base, current) !== 0) throw new Error('Patch baseVersion must equal the current version');
  if (compareVersion(target, base) <= 0) throw new Error('Patch version must be greater than baseVersion');
  ...
  for (const path of paths) {
    ...
    if (Object.prototype.hasOwnProperty.call(verifiedFiles, canonicalPath)) throw new Error(`Duplicate canonical patch asset path: ${canonicalPath}`);
    ...
    if (typeof entry.sha256 !== 'string' || !SHA256_HEX.test(entry.sha256)) throw new Error(`Patch file ${path} SHA-256 must be 64 lowercase hex characters`);
  ...
  if (signature.alg !== PATCH_ALGORITHM || key.alg !== PATCH_ALGORITHM) throw new Error(`Patch signature algorithm must be ${PATCH_ALGORITHM}`);
  if (typeof signature.keyId !== 'string' || signature.keyId !== key.keyId) throw new Error('Patch signature key is not trusted');
  const jwk = asObject(key.jwk, 'Trusted patch public JWK');
  let publicKey;
  try { publicKey = await subtleCrypto().importKey('jwk', jwk, { name:'ECDSA', namedCurve:'P-256' }, false, ['verify']); }
  catch { throw new Error('Trusted patch public key is invalid'); }
  let signatureBytes;
  try { signatureBytes = base64urlToBytes(signature.value); } catch (error) { throw new Error(`Patch signature is invalid: ${error.message}`); }
  const validSignature = await subtleCrypto().verify({ name:'ECDSA', hash:'SHA-256' }, publicKey, signatureBytes, encoder.encode(canonicalPatchPayload(source)));
  if (!validSignature) throw new Error('Patch signature verification failed');
  return { schema:PATCH_SCHEMA, baseVersion:source.baseVersion, version:source.version, files:verifiedFiles };
}

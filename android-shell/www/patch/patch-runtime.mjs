...
function validateManifest(manifest, currentVersion) {
  if (!manifest || typeof manifest !== 'object') throw new Error('Patch manifest is invalid');
  const { latestVersion, baseVersion, patchUrl, sha256, size } = manifest;
  if (!validVersion(latestVersion) || !validVersion(baseVersion)) throw new Error('Patch manifest version is invalid');
  ...
  if (latestVersion !== currentVersion && baseVersion !== currentVersion) throw new Error('Patch manifest baseVersion mismatch');
  return { latestVersion, baseVersion, patchUrl, sha256:sha256.toLowerCase(), size:size ?? null };
}
...
export async function fetchLatestPatch({
  ...
  try {
    ...
    throw new Error('Patch download JSON is invalid');
  }
...
export async function mountSnapshot(snapshot, {
  ...
  let rules;
  let vocabulary;
  try { rules = JSON.parse(assets['rules.json']); vocabulary = JSON.parse(assets['vocabulary.json']); }
  catch (error) { throw new Error(`Snapshot data JSON is invalid: ${error.message}`); }
...
export async function startPatchRuntime({ documentRef = globalThis.document, fetchImpl = globalThis.fetch, indexedDB = globalThis.indexedDB, trustedBrain = null } = {}) {
  ...
  const patchButton = documentRef.getElementById('patch-latest');
  ...
  const patchUpdater = Object.freeze({
    async updateLatest() {
      ...
      try {
        ...
        const fetched = await fetchLatestPatch({
          currentVersion:before.version,
          fetchImpl,
          verifyDownloadedBundle:(bundle) => verifyPatchBundle(bundle, { currentVersion:before.version, trustedKey }),
        });
        if (fetched.status === 'LATEST') return fetched;
        const applied = await applyPatchBundle(fetched.bundle, { store, trustedKey });
        return { status:'ACTIVATED', ...applied };
  ...
  const render = async (snapshot) => {
    await cleanup();
    cleanup = await mountSnapshot(snapshot, { root, documentRef, trustedBrain, patchUpdater });
  ...
  const runOneTap = async () => {
    ...
    try {
      ...
      try { await render(result.current); }
      catch (mountError) { await store.rollback(); await render(result.before); throw new Error(`Patch mount failed and was rolled back: ${mountError.message}`); }
      status.textContent = `Patch ${result.current.version} ใช้งานแล้ว`;
      return result;
  ...
  fileInput.addEventListener('change', async () => {
    ...
    try {
      ...
      try { await render(current); }
      catch (mountError) { await store.rollback(); await render(before); throw new Error(`Patch mount failed and was rolled back: ${mountError.message}`); }
      status.textContent = `Patch ${current.version} ใช้งานแล้ว`;
    } catch (error) {
      if (attemptedVersion) {
        const meta = await store.readMeta().catch(() => null);
        if (meta?.currentVersion === attemptedVersion && meta.previousVersion) { await store.rollback().catch(() => undefined); await render(await store.readCurrent()).catch(() => undefined); }
      }
  ...
  rollbackButton.addEventListener('click', async () => {
    ...
    try { await store.rollback(); const current = await store.readCurrent(); await render(current); status.textContent = `ย้อนกลับเป็น ${current.version} แล้ว`; }
    catch (error) { status.textContent = `ย้อน Patch ไม่สำเร็จ: ${error.message}`; }
  });

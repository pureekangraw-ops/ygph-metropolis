...
test('parses APK signer certificate SHA-256 and normalizes punctuation', () => {
  ...
  assert.equal(parseApksignerCertificateSha256(output), SIGNER);
...
test('fails closed on package, signer, versionCode, or versionName mismatch', () => {
  ...
  assert.throws(() => assertApkIdentity({ ...expected, versionName: 'legacy' }, expected), /APK_VERSION_NAME_MISMATCH/);
});

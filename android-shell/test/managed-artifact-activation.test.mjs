...
function waiting(){return {packageName:PACKAGE,versionName:'1.0.5',versionCode:1006,sha256:HASH,apkUrl:META.apkUrl,signerSha256:SIGNER};}
function bridge({installedCode=1005,onInstaller=()=>{}}={}){
  return {
    async getInstalledIdentity(){return {packageName:PACKAGE,versionName:installedCode===1006?'1.0.5':'1.0.4',versionCode:installedCode};},
    async downloadApk(options){assert.equal(options.versionCode,1006);return {sha256:HASH,sizeBytes:42};},
    async inspectApk(options){assert.equal(options.versionCode,1006);return {packageName:PACKAGE,versionName:'1.0.5',versionCode:1006,signerSha256:SIGNER};},
...
test('verified candidate is persisted before Android installer handoff',async()=>{
  ...
  assert.equal(saved.packageName,PACKAGE);
...
test('resume reads installed identity: old vc is retryable, target vc closes the handoff',async()=>{
  const {createAppUpdater,UPDATE_WAITING_STORAGE_KEY}=await module();
  const storage=memoryStorage({[UPDATE_WAITING_STORAGE_KEY]:JSON.stringify(waiting())});

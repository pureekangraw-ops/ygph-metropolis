...
- **Calculation Authority reuse — Enhancement opportunity.** `greenfield/calculation-authority.mjs` already proves a useful separation between Ledger cash, generated Store/Ride activity, receivables, obligations, and Calendar comparisons. Task 7 should inspect/reuse this authority before inventing a second Manual calculation brain. Its current `generated` projection is specifically Store + Ride, while the agreed Manual Income family also includes Other Income from Ledger, so `generated.totalSatang` must not silently become the complete Manual Income total. Treat existing `semanticWarnings` as evidence that unresolved ownership stays `VERIFY` rather than being silently promoted into truth.
...
- **Balance-reconciliation metadata preservation — Blind-spot guardrail.** `buildBalanceAdjustmentWorkflow()` already supplies `balanceBeforeSatang`, `balanceAfterSatang`, and `adjustmentReason`, but the current generic `LEDGER_CREATE_TRANSACTION` record builder does not persist those fields. A final-balance readback alone therefore does not prove the reconciliation story/audit context survived. When Ledger/report/history work reaches balance adjustment, prove that the intended reconciliation metadata survives the workflow → command → durable record path rather than only proving the arithmetic result.
...
- **Idempotent recovery semantic identity — Blind-spot guardrail.** `executeMultiGroupCommands()` already has a useful `RECOVERED` path, but current `commandLog` entries retain command ID/domain/type/revision/time and not the original payload or semantic fingerprint. The recovery gate currently checks that incoming idempotency keys were committed, not that the retried parameters still describe the same operation. Before Manual mutations reuse this path, prove that key reuse with changed target/amount/parameters fails as a mismatch rather than being reported as recovery of an operation that never ran. Keep identical-retry recovery behavior intact.
- **Durable schema-migration writeback — Blind-spot guardrail.** Current `readEncryptedState()` decrypts and applies `migrateGreenfieldState()` in memory, while the raw encrypted vault is only rewritten by a later durable mutation. `exportGreenfieldBackup()` copies the raw vault rather than the migrated in-memory object. Before APK/data-upgrade acceptance, prove `old durable schema → new runtime migration → explicit durable commit/readback → backup/restore` rather than treating a successful read-time migration as proof that the stored vault itself has advanced.
- **Restore overwrite surface — Regression/surface guardrail.** The backup layer and tests already require one explicit overwrite decision when valid backup data would replace an initialized store, and earlier product guidance also says replacement is the case that needs confirmation. The current direct `ui/app.mjs` restore handler calls `runtime.restoreBackup(backup)` without an `allowOverwrite` completion path, so an initialized store can stop at `GREENFIELD_RESTORE_CONFIRM_REQUIRED`. Treat this as a surface/regression proof item: preserve verify-before-replace, expose exactly one explicit replacement decision, then use the existing restore/readback authority rather than weakening the overwrite guard.
...
- **Portable backup trust boundary — Enhancement/security guardrail.** Current runtime `exportBackup()` supplies the active vault passphrase as `recoveryKey`, and the downloaded backup therefore carries the decrypt secret in the same JSON object as the encrypted vault. This is a self-contained recovery package, not confidentiality against a party who possesses the complete file. Preserve portable convenience only with explicit trust semantics and user-facing copy; do not collapse it with a secret-separated backup under a generic security claim such as “encrypted backup”.

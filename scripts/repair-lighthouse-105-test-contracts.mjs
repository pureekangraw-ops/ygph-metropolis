...
for (const [from,to] of [
  ...
  finalGate = finalGate.split(from).join(to);
}
...
let bridgeTest = readFileSync('tests/greenfield-manual-context-bridge.test.cjs','utf8');
bridgeTest = bridgeTest
  .replace("node.dataset.primaryAction === 'Complete'", "node.dataset.primaryAction === 'COMPLETED'")
  .replace("node.dataset.primaryAction === 'Complete'", "node.dataset.primaryAction === 'COMPLETED'");

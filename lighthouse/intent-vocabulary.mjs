...
function makeToken({ rawText, start, end, role, canonical = null, state = 'KNOWN', possibleRoles = [], optional = false, resolvedValue = null }) {
  return freeze({
...
function formAllowedAt(rawText, index, form) {
  if (form === 'ขอ' && rawText.startsWith('ของ', index)) return false;
  return true;
}
...
function matchKnownWord(rawText, index) {
  for (const entry of WORDS) {
    for (const form of entry.forms) {
      if (formAllowedAt(rawText, index, form) && rawText.startsWith(form, index)) return { entry, form };
    }
  }
  return null;
}
  ...
  while (index < rawText.length) {
    ...
    if (date) {
      ...
      tokens.push(makeToken({
        rawText, start:index, end, role:'TEMPORAL', possibleRoles:['TEMPORAL'],
        resolvedValue:{ dayText:date[0].slice('วันที่'.length) },
      }));
      index = end;
      continue;
    }
    ...
    tokens.push(makeToken({ rawText, start:index, end, role:'PENDING', state:'UNKNOWN' }));
    index = end;
  }

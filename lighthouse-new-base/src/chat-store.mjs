...
function emptyDocument(conversationId = null, at = null) {
  return {
    schemaVersion:1,
    conversation:{ id:conversationId, createdAt:at, updatedAt:at, archived:false },
    messages:[], drafts:[], work:[], events:[], changeMarkers:{}, submitTokens:{},
  };
}
...
export function createChatStore({ storage = defaultStorage(), key = DEFAULT_KEY, idFactory = defaultIdFactory, now = () => new Date().toISOString() } = {}) {
  ...
  function read() {
    ...
    return raw ? clone(JSON.parse(raw)) : emptyDocument();
  }
  function ensureDocument() {
    const document = read();
    if (document.conversation?.id) return document;
    const at = now();
  ...
  function updateDocument(mutator) {
    ...
    updated.conversation.updatedAt = now();
  ...
  function commitUserMessage(rawText, { submitToken = null } = {}) {
    ...
    const work = {
      id:idFactory('work'), messageId:message.id, kind:'INTERPRET', status:'WAITING', attempts:0,
